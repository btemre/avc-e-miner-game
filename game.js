...
function sanitizeCloudSavePayload(obj) {
    ...
    const base = {
        money: 0, pickLvl: 1, pickUpgradeCost: 100,
...
function getUpgradeCost(baseCost) {
    return state.currentRole === 'engineer' ? Math.floor(baseCost * 0.8) : baseCost;
}
...
function actionUpgradePick() {
    ...
    if (state.money >= cost) {
        state.money -= cost;
        state.pickLvl++;
        state.pickUpgradeCost = Math.floor(state.pickUpgradeCost * 3.0);
